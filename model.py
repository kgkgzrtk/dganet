...
class dganet(object):
    ...
    def build(self):
        ...
        tl_sum = tf.scalar_summary("training_loss", self.loss/self.batch_size)
        vl_sum = tf.scalar_summary("validation_loss", self.loss/self.batch_size)
        self.t_sum_op = tf.merge_summary([tl_sum])
        self.v_sum_op = tf.merge_summary([vl_sum])
    ...
    def inference(self, input_):
        with tf.name_scope('conv') as scope:
            ...
            y_c0 = conv(input_, dim, c=7, name='c0', bn=False)
            y_c1 = tf.nn.relu(conv(tf.nn.relu(batch_norm(y_c0)), dim * 2, c=7, name='c1'))
            y_p1 = pool(y_c1, name='pool1')
            y_c2 = tf.nn.relu(conv(y_p1, dim * 2, c=5, name='c2'))
            y_c3 = tf.nn.relu(conv(y_c2, dim * 4, c=5, k=2, name='c3'))
        ...
        with tf.name_scope('gen') as scope:
            ...
            #y_dc1 = tf.nn.dropout(y_dc1, 0.5)
            y_dc2 = tf.nn.relu(resize_conv(y_dc1, [self.batch_size, self.w_range[2], self.w_range[2], dim * 2], c=3, name='dc2'))
            y_dc3 = tf.nn.relu(resize_conv(y_dc2, [self.batch_size, self.w_range[1], self.w_range[1], dim * 2], c=5, name='dc3'))
            y_dc4 = tf.nn.relu(resize_conv(y_dc3, [self.batch_size, self.w_range[0], self.w_range[0], dim], c=5, name='dc4'))
            #y_dc4 = tf.nn.dropout(y_dc4, 0.5)
            y_in = tf.add(y_dc4, y_c0)
            y_dc5 = tf.nn.relu(resize_conv(y_in, [self.batch_size, self.w_range[0], self.w_range[0], dim], c=7, name='dc5'))
            #y_dc5 = tf.nn.dropout(y_dc5, 0.5)
            y_dc6 = tf.nn.sigmoid(resize_conv(y_dc5, [self.batch_size, self.w_range[0], self.w_range[0], 1], c=7, name='dc6', bn=False))
    ...
    def train(self, i_lr=0.00002, d_lr=0.00002, train_epoch=1000):
        ...
        op_loss = self.loss + 100.*tf.add_n(tf.get_collection('w_loss')) 
        i_train_op = tf.train.AdamOptimizer(1e-4).minimize(self.loss, var_list=list(self.c_vars + self.g_vars))
        #d_train_op = tf.train.AdamOptimizer(1e-4).minimize(self.d_loss, var_list=self.d_vars)
        
        tf.initialize_all_variables().run()
        self.load_model('dganet_e00202.model')
        ...
        for epoch in range(211,train_epoch):
    ...
    def load_image(self):
        ...
        data_list = zip(train_image, train_depth)
        test_list = [data_list[i] for i in range(len(data_list)) if i%7==0]
        train_list = [data_list[i] for i in range(len(data_list)) if i%7!=0]
        train_image, train_depth = zip(*train_list)
        test_image, test_depth = zip(*test_list)
