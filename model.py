...
class dganet(object):
    ...
    def inference(self, input_):
        with tf.variable_scope('gen') as scope:
            ...
            y_c1 = tf.nn.relu(conv(tf.nn.relu(batch_norm(y_c0)), dim * 2, c=3, k=1, name='c1-0'))
            y_c1 = tf.nn.relu(conv(y_c1, dim * 2, c=3, k=2, name='c1-1'))
            y_c2 = tf.nn.relu(conv(y_c1, dim * 4, c=3, k=1, name='c2-0'))
            y_c2 = tf.nn.relu(conv(y_c2, dim * 4, c=3, k=2, name='c2-1'))
            y_c3 = tf.nn.relu(conv(y_c2, dim * 8, c=3, k=1, name='c3-0'))
            y_c3 = tf.nn.relu(conv(y_c3, dim * 8, c=3, k=2, name='c3-1'))
            y_c4 = tf.nn.relu(conv(y_c3, dim * 16, c=3, k=1, name='c4-0'))
            y_c4 = tf.nn.relu(conv(y_c4, dim * 16, c=3, k=2, name='c4-1'))
            y_c5 = tf.nn.relu(conv(y_c4, dim * 32, c=3, k=1, name='c5-0'))
            y_c5 = tf.nn.relu(conv(y_c5, dim * 32, c=3, k=2, name='c5-1'))


            y_dc0 = tf.nn.relu(resize_conv(y_c5, [self.batch_size, self.w_range[4], self.w_range[4], dim * 16], c=3, name='dc0'))
            y_dc0 = tf.nn.relu(resize_conv(y_dc0, [self.batch_size, self.w_range[4], self.w_range[4], dim * 16], c=3, name='dc0_'))
            y_in1 = tf.concat([y_dc0, y_c4],3)
            y_dc1 = tf.nn.relu(resize_conv(y_in1, [self.batch_size, self.w_range[3], self.w_range[3], dim * 8], c=3, name='dc1'))
            y_dc1 = tf.nn.relu(resize_conv(y_dc1, [self.batch_size, self.w_range[3], self.w_range[3], dim * 8], c=3, name='dc1_'))
            y_in2 = tf.concat([y_dc1, y_c3],3)
            y_dc2 = tf.nn.relu(resize_conv(y_in2, [self.batch_size, self.w_range[2], self.w_range[2], dim * 4], c=3, name='dc2'))
            y_dc2 = tf.nn.relu(resize_conv(y_dc2, [self.batch_size, self.w_range[2], self.w_range[2], dim * 4], c=3, name='dc2_'))
            y_in3 = tf.concat([y_dc2, y_c2],3)
            y_dc3 = tf.nn.relu(resize_conv(y_in3, [self.batch_size, self.w_range[1], self.w_range[1], dim * 2], c=3, name='dc3'))
            y_dc3 = tf.nn.relu(resize_conv(y_dc3, [self.batch_size, self.w_range[1], self.w_range[1], dim * 2], c=3, name='dc3_'))
            y_in4 = tf.concat([y_dc3, y_c1],3)
            y_dc4 = tf.nn.relu(resize_conv(y_in4, [self.batch_size, self.w_range[0], self.w_range[0], dim], c=3, name='dc4'))
            y_dc4 = tf.nn.relu(resize_conv(y_dc4, [self.batch_size, self.w_range[0], self.w_range[0], dim], c=3, name='dc4_'))
            y_in5 = tf.concat([y_dc4, y_c0],3)
            y_dc5 = tf.nn.relu(resize_conv(y_in5, [self.batch_size, self.w_range[0], self.w_range[0], dim], c=3, name='dc5'))
    ...
    def train(self, i_lr=0.00002, d_lr=0.00002, train_epoch=1000):
        ...
        g_train_op = tf.train.AdamOptimizer(3e-4).minimize(self.g_loss, var_list=self.g_vars)
        ...
        for epoch in range(train_epoch):
            ...
            for i in range(len(self.train_image)//batch_size):
                ...
                _, tl, gl = self.sess.run([g_train_op, self.loss, self.g_loss], feed_dict = train_feed)
                _, dl = self.sess.run([d_train_op, self.d_loss], feed_dict = train_feed)
