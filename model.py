...
class dganet(object):
    ...
    def inference(self, input_):
        ...
        with tf.name_scope('gen') as scope:
            #generator
            y_dc0 = tf.nn.relu(deconv(y_p5, [self.batch_size, self.w_range[3], self.w_range[3], dim * 8], c=3, name='dc0'))
            y_dc1 = tf.nn.relu(deconv(y_dc0, [self.batch_size, self.w_range[3], self.w_range[3], dim * 4], c=3, k=1, name='dc1'))
            y_dc1 = tf.nn.dropout(y_dc1, 0.5)
            y_dc2 = tf.nn.relu(deconv(y_dc1, [self.batch_size, self.w_range[2], self.w_range[2], dim * 2], c=3, name='dc2'))
            y_dc3 = tf.nn.relu(deconv(y_dc2, [self.batch_size, self.w_range[1], self.w_range[1], dim * 2], c=5, name='dc3'))
            y_dc4 = tf.nn.relu(deconv(y_dc3, [self.batch_size, self.w_range[0], self.w_range[0], dim], c=7, name='dc4'))
            y_dc4 = tf.nn.dropout(y_dc4, 0.5)
            y_in = tf.add(y_dc4, y_c0)
            y_dc5 = tf.nn.relu(deconv(y_in, [self.batch_size, self.w_range[0], self.w_range[0], dim], c=11, k=1, name='dc5'))
    ...
    def train(self, i_lr=0.00002, d_lr=0.00002, train_epoch=5000):
        ...
        for epoch in range(train_epoch):
            ...
            for i in range(len(self.train_image)/batch_size):
                batch = batch_size*i
                train_feed = {self.x: self.train_image[batch:batch+batch_size], self.y_: self.train_depth[batch:batch+batch_size]}

                for k in range(self.k_num): self.sess.run(i_train_op, feed_dict = train_feed)
