...
def conv(image, out_dim, name, c=4, k=2, stddev=0.01, bn=True):
    with tf.variable_scope(name) as scope:
        ...
        pad_image = tf.pad(image, [[0, 0], [1, 1], [1, 1], [0, 0]])
...
def resize_conv(image, output_shape, name, c=4, k=2, bn=True):
    image = tf.image.resize_images(image, [output_shape[1]*2, output_shape[2]*2])
